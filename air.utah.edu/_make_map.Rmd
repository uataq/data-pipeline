...
pretty_site_names <- data_frame(csp='Castle Peak',
                                ...
                                haw='Hawthorne (DAQ)',
                                wnd='Wendover')
url_link <- data_frame(csp='http://air.utah.edu/s/view/meas/?site_id=',
                       dbk=csp,
                       fru=csp,
                       hdp=csp,
                       heb=csp,
...
for (i in 1:nrow(dat)) {
  ...
  # if (short_species[[tracer]] %in% c('O3_ppbv', 'PM25_ugm3')) {
    # pop <- sapply(pop, paste, 'Data in collaboration with')
  # }
}
...
if (nrow(dat) > 1) {
  for (site in unique(dat$site)){
    ...
    mpop<- paste(sep='<br>',
                 ...
                 paste('Last updated:  ', format(mdat$Time_UTC, tz = 'America/Denver', '%Y-%m-%d %H:%M %Z')),
                 a(target='_blank',
                   href=url_link[[site]],
                   tags$button(type='button',
                               class='btn btn-danger btn-block',
