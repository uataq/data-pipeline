...
teledyne_t500u_qaqc <- function() {
  ...
  nd$QAQC_Flag[with(nd, Samp_T_C < 42 | Samp_T_C > 48)] <- -124
  nd$QAQC_Flag[with(nd, ARef_L_mm < 400 | ARef_L_mm > 1100)] <- -125
  nd$QAQC_Flag[filter_warmup(nd, cooldown = '30M', warmup = '45M')] <- -126
