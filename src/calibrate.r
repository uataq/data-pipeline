...
calibrate <- function(time, meas, known, er_tol = 100, drift_tol = 100, dt_tol = 18000) {
  ...
  std_uniq <- unique(known[known != -10])
  ...
  # QAQC ---------------------------------------------------------------------------------
  # Remove long-term standard to prevent self-calibration
  ...
  # exceeds drift_tol.
  delta <- abs(rbind(rep.int(NA, ncol(stdm)), stdm[2:N, ] - stdm[1:(N-1), ]))
  mask <- delta > (drift_tol / stdtelap)
  mask[is.na(mask)] <- F
  stdk[mask] <- NA
  qaqc[rowSums(mask) > 0] <- 5

  # Invalidate periods with longer tsthan dt_tol seconds between calibrations.
  # First LTS at row id 1476
